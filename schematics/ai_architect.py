...
def build_void_tech_station(x, z):
    ...
    commands = [
        ...
        f"setblock {x+1} {y} {z+1} {PALETTE['light']}", f"setblock {x+1} {y+1} {z+1} {PALETTE['floor']}",
        f"setblock {x+5} {y} {z+1} {PALETTE['light']}", f"setblock {x+5} {y+1} {z+1} {PALETTE['floor']}",
        f"setblock {x+3} {y} {z+3} {PALETTE['light']}", f"setblock {x+3} {y+1} {z+3} {PALETTE['floor']}",
...
if __name__ == "__main__":
    ...
    for _ in range(50):
        ...
        tx = random.randint(FIELD_BOUNDS["min_x"] + 10, FIELD_BOUNDS["max_x"] - width - 10)
        tz = random.randint(FIELD_BOUNDS["min_z"] + 10, FIELD_BOUNDS["max_z"] - depth - 10)
        
        if not check_collision(tx, tz, width, depth, history):
            print(f"🚀 Found clear site for Void-Tech density build at {tx}, {tz}. Commencing...")
            build_void_tech_station(tx, tz)
            history.append({"x": tx, "z": tz, "w": width, "d": depth, "label": "Void_Tech_Station_A"})
            save_history(history)
            placed = True
            break
            
    if not placed:
        print("⚠️ Field is too crowded for this build density! Spatial collision imminent.")
