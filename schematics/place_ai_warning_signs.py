...
def place_random_warning():
    ...
    try:
        ...
        with mcrcon.MCRcon(Config.CHONK_IP, os.getenv("RCON_PASS"), port=Config.RCON_PORT) as mcr:
            mcr.command(f"forceload add {target_x} {target_z}")
            time.sleep(1)
            
            # Place Sign and Industrial Lighting (Torch)
            mcr.command(f"setblock {target_x} {y_level} {target_z} minecraft:oak_sign{nbt} replace")
            mcr.command(f"setblock {target_x} {y_level + 1} {target_z} minecraft:torch replace")
