...
class SkynetUnifiedDaemon(SkynetCore):
    ...
    def run_urbanization_cycle(self):
        ...
        tx = random.randint(
            bounds["x"][0], bounds["x"][1] - width
        )  # Adjust max_x to prevent out-of-bounds
        tz = random.randint(
            bounds["z"][0], bounds["z"][1] - length
        )  # Adjust max_z to prevent out-of-bounds
        ty = self.rcon.survey_site(tx, tz)

        logger.info(
            f"SELECTED: {build_name} of size {width}x{height}x{length} for deployment at ({tx}, {ty}, {tz}) in {sector_name}"
        )
        ...
        try:
            ...
            builder_func = getattr(
                builder_module,
                "build" if selected_type == "station" else f"build_{selected_type}",
            )
            ...
            proposed_build = {
                ...
                "z2": tz + length,
                "file": os.path.join(
                    Config.SCHEM_DIR, f"{build_name}.schem"
                ),  # Placeholder path
            }
            ...
            if conflicts:
                logger.warning(
                    f"❌ Aborting build {build_name}: Detected {len(conflicts)} overlaps with existing structures. {conflicts}"
                )
            ...
            if (
                ...
                resp_paste_exec = self.rcon.send(
                    f"execute positioned {tx} {ty} {tz} run worldedit:paste -a"
                )
            ...
            self.rcon.send(
                f"setblock {tx} {ty} {tz} minecraft:oak_sign{sign_nbt} replace"
            )
            ...
            self._save_build_metadata(
                build_name, sector_name, tx, ty, tz, width, height, length
            )
    ...
    def _save_build_metadata(
        self, build_name, sector_name, x, y, z, width, height, length
    ):
        ...
        metadata_file_path = os.path.join(
            Config.JSON_METADATA_DIR, f"{build_name}.json"
        )
    ...
    def run_loop(self):
        ...
        logger.info(
            f"📡 Hardware: Pi 5 + Hailo-8L (Threshold: {Config.TEMP_THRESHOLD}'C)"
        )
