...
class AdaptiveMutator:
    ...
    def apply_infection(self, x, z, state, score):
        ...
        if state == "INTENSION_HIGH":
            ...
            cmds.append(f"say [Skynet] INTENSION_HIGH detected. Initiating Aggressive Reclamation.")
            ...
            for _ in range(intensity):
                ...
                cmds.append(f"setblock {rx} {ry} {rz} {INFECTED_PALETTE['infection_core']} replace")
                if random.random() > 0.7:
                    cmds.append(f"setblock {rx} {ry+1} {rz} {INFECTED_PALETTE['reclamation_anchor']} replace")
            ...
            # Sparse mutation
            intensity = 5 + score * 2
            cmds.append(f"say [Skynet] Class B Incursion detected. Spreading Mycelial Mutation.")
            for _ in range(intensity):
                rx = x + random.randint(-radius, radius)
                rz = z + random.randint(-radius, radius)
