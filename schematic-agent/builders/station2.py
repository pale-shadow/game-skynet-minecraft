...
def build(schem, config):
    ...
    """
    w, h, l = config['dimensions']['width'], config['dimensions']['height'], config['dimensions']['length']
    ...
    if l > 15: pillar_locs.extend([(1, l//2), (w-2, l//2)])
    ...
    for px, pz in pillar_locs:
        for py in range(1, h - 2):
            ...
            schem.setBlock((px+1, py, pz), f"{palette['trim']}[facing=west,half=bottom]")
            schem.setBlock((px-1, py, pz), f"{palette['trim']}[facing=east,half=bottom]")
            schem.setBlock((px, py, pz+1), f"{palette['trim']}[facing=north,half=bottom]")
            schem.setBlock((px, py, pz-1), f"{palette['trim']}[facing=south,half=bottom]")
    ...
    for x in range(w):
        for z in range(l):
            ...
            if (x == 1 or x == w-2) and z % 5 == 0:
                ...
                schem.setBlock((x, h-2, z), palette.get('lighting', 'minecraft:pearlescent_froglight'))
                # Decorative framing around the light
                schem.setBlock((x+1, h-2, z), "minecraft:purpur_stairs[facing=west]")
                schem.setBlock((x-1, h-2, z), "minecraft:purpur_stairs[facing=east]")
