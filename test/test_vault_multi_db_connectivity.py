...
def test_vault_multi_db_connectivity(db_info):
    ...
    try:
        ...
        pytest.fail(f"Connectivity failure for {db_info['name']} as {db_info['user']}: {e}")
...
def test_skynet_vault_schema_integrity():
    ...
    with conn.cursor() as cursor:
        ...
        required_columns = [
            "id", "name", "x", "y", "z", 
            "width", "depth", "schematic_name", 
            "ai_hardware", "timestamp"
        ]
        
        for req in required_columns:
            assert req in columns, f"Missing mandatory column '{req}' in skynet_vault.build_history"
