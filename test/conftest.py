...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "host(name): mark test to run only on a specific host")
    config.addinivalue_line("markers", "database: mark test as requiring a database connection")
    config.addinivalue_line("markers", "world_state: mark test as requiring Minecraft .mca files")
...
def pytest_runtest_setup(item):
    ...
    if host_marker:
        ...
        if current_host != required_host:
            pytest.skip(f"Skipped: Test requires host '{required_host}', but running on '{current_host}'")
