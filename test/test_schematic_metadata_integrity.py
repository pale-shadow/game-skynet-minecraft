...
def test_metadata_integrity(schem_path):
    ...
    required_top_level = ["build_id", "provenance", "hardware_telemetry", "spatial_data", "performance_impact"]
    for field in required_top_level:
        assert field in data, f"Missing required top-level field '{field}' in {json_path}"
    ...
    assert "dimensions" in spatial, f"Missing 'dimensions' in spatial_data for {json_path}"
    ...
    for axis in ["x", "y", "z"]:
        ...
        assert isinstance(origin[axis], (int, float)), f"Origin axis '{axis}' must be numeric in {json_path}"
        
    for dim in ["width", "height", "length"]:
        assert dim in dimensions, f"Missing dimension '{dim}' in {json_path}"
        assert isinstance(dimensions[dim], (int, float)), f"Dimension '{dim}' must be numeric in {json_path}"
    ...
    assert inf_node != "unknown", f"Inference node is 'unknown' in {json_path}. Must be attributed to a valid host."
    assert inf_node in ["Skynet-Pi5-Hailo8L", "Pi5 / NVMe (Stargate)", "retroactive_generator"], f"Invalid inference node '{inf_node}' in {json_path}"
