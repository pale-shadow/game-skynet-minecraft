...
def push_build_to_chonk(commands):
    try:
        with mcrcon.MCRcon(
            os.getenv("CHONK_IP"), os.getenv("RCON_PASS"), port=25575
        ) as mcr:
            for cmd in commands:
                mcr.command(cmd)
...
def get_v6_hailo_iterated_hub():
    ...
    cmds.append(
        f"fill {x-3} {y} {z-3} {x+3} {y+15} {z+3} minecraft:polished_deepslate_bricks"
    )
    ...
    for sy in [2, 6, 10]:
        ...
        cmds.append(
            f"setblock {x+5} {y+sy} {z} minecraft:oxidized_copper_bulb[lit=false]"
        )

    # PHASE 4: Industrial Detailing (Minecarts & Chains)
    cmds.append(
        f"summon minecart {x-10} {y+10} {z} {{CustomName:'\"Hailo-NPU-Logistics\"'}}"
    )
    cmds.append(
        f"fill {x-5} {y+15} {z-5} {x+5} {y+15} {z+5} minecraft:dark_prismarine_stairs"
    )
    cmds.append(
        f"fill {x} {y+16} {z} {x} {y+25} {z} minecraft:chain"
    )  # Suspension Cable

    cmds.append(
        f"say [Hailo-NPU] V6 Architecture deployed at {x} {z}. Iteration cycles complete."
    )
    return cmds
