...
def deploy_batch():
    ...
    if not rcon_pass:
        print(
            "Error: RCON_PASS is not set. Please set the environment variable or ensure Config.RCON_PASS is valid."
        )
        return
    ...
    try:
        ...
        with MCRcon(rcon_ip, rcon_pass, port=rcon_port) as mcr:
            for build in build_queue:
                ...
                width, depth = build.get("w", 10), build.get(
                    "d", 10
                )  # Default dimensions
                ...
                if x is not None and z is not None:
                    print(
                        f"==> Deploying {build.get('name', 'Unnamed Build')} to {x}, {y}, {z}"
                    )
                    ...
                    if not schematic_name:
                        print(
                            f"Skipping {build.get('name', 'Unnamed Build')}: Missing 'schematic_name'."
                        )
                        continue
                    ...
                    engine.history.append(
                        {"name": build["name"], "x": x, "y": y, "z": z}
                    )
                else:
                    print(
                        f"Skipping {build.get('name', 'Unnamed Build')}: Could not determine placement coordinates."
                    )
