...
def get_v5_kinetic_terminal():
        ...
        for dz in range(-15, 16):
            if random.random() > 0.7:
                ...
                if random.random() > 0.5:
                    commands.append(
                        f"setblock {x+dx} {y} {z+dz} minecraft:purple_carpet"
                    )
    ...
    # 2. THE RAIL DECK
    commands.append(
        f"fill {x-12} {y+4} {z-4} {x+12} {y+4} {z+4} minecraft:polished_tuff"
    )
    commands.append(
        f"fill {x-12} {y+5} {z-1} {x+12} {y+5} {z+1} minecraft:chiseled_tuff"
    )  # Upgraded Track Bed
    ...
    commands.append(
        f"fill {x-2} {y} {z-2} {x+2} {y+12} {z+2} minecraft:crying_obsidian"
    )
    commands.append(
        f"fill {x-1} {y+1} {z-1} {x+1} {y+11} {z+1} minecraft:amethyst_block"
    )
    ...
    commands.append(
        f"setblock {x+4} {y+5} {z} minecraft:oxidized_copper_bulb[lit=false]"
    )
    commands.append(f"setblock {x-3} {y+5} {z} minecraft:sculk_sensor")
    commands.append(
        f"setblock {x-4} {y+5} {z} minecraft:oxidized_copper_bulb[lit=false]"
    ...
    for px, pz in [(-10, -10), (10, -10), (-10, 10), (10, 10)]:
        commands.append(
            f"fill {px+x} {y} {pz+z} {px+x} {y+15} {pz+z} minecraft:purpur_pillar"
        )
        commands.append(f"setblock {px+x} {y+16} {pz+z} minecraft:lightning_rod")

    # 5. MINE CART DEPLOYMENT
    commands.append(
        f"summon minecart {x+5} {y+7} {z} {{CustomName:'\"Skynet-V5-Express\"'}}"
    )

    commands.append(
        f"say [Skynet] V5 Kinetic Terminal Online. Reaction field active at {x} {z}."
    )

    # BlueMap POI
    create_bluemap_marker(
        f"kinetic_{int(time.time())}",
        "V5 Kinetic Terminal",
        x,
        y + 10,
        z,
