...
class VisionLiteOverseer:
    ...
    def scan_area(self, x, z, radius=8):
        ...
        incursion_points = 0
        samples = (
            15  # Sample 15 random points in the area to simulate vision processing
        )
        ...
        for _ in range(samples):
            ...
            for ry in range(y_base - 1, y_base + 8):
                ...
                for block in self.human_blocks:
                    res = self.rcon.send(
                        f"execute if block {rx} {ry} {rz} {block}", silent=True
                    )
                    if res and (
                        "Test passed" in str(res)
                        or "Matched" in str(res)
                        or str(res).strip() == "1"
                    ):
                        incursion_points += 1
                        break
