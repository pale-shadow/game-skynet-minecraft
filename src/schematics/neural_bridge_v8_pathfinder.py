...
def generate_v8_pathfinded_bridge(path):
    ...
    for i, node in enumerate(path):
        ...
        # --- PHASE 1: THE DECK ---
        cmds.append(
            f"fill {final_x-2} {y_base} {final_z-2} {final_x+2} {y_base} {final_z+2} {p['base']}"
        )
        ...
        if i % 10 == 0:
            cmds.append(
                f"setblock {final_x} {y_base-1} {final_z} minecraft:redstone_block"
            )

        # Suspension Pylons (Thin)
        if i % 20 == 0:
            cmds.append(
                f"fill {final_x} {y_base-1} {final_z} {final_x} {y_base-10} {final_z} minecraft:end_rod"
            )
            cmds.append(f"setblock {final_x} {y_base-11} {final_z} {p['glow']}")

    cmds.append(
        f"say [Hailo-NPU] V8 Neural Pathfinder Bridge link established. Path Optimized."
    )
    return cmds
...
if __name__ == "__main__":
    ...
    if len(history) < 2:
        ...
        print(
            f"🧠 Calculating path from {start_node['label']} to {end_node['label']}..."
        )
        ...
        if path:
            ...
            create_bluemap_marker(
                f"bridge_v8_{int(time.time())}",
                "Neural Pathfinded Bridge",
                mid_node[0],
                FIELD_BOUNDS["y"] + 15,
                mid_node[1],
